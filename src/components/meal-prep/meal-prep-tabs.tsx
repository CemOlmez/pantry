...
import type { MealPrepTab } from "./types";

/* ------------------------------------------------------------------ */
/*  Props                                                              */
/* ------------------------------------------------------------------ */

type MealPrepTabsProps = {
  activeTab: MealPrepTab;
  onTabChange: (tab: MealPrepTab) => void;
  labels: {
    discover: string;
    favorites: string;
    myMealPreps: string;
...
export function MealPrepTabs({ activeTab, onTabChange, labels, counts }: MealPrepTabsProps) {
  const tabs: { key: MealPrepTab; label: string; count?: number }[] = [
    { key: "discover", label: labels.discover, count: counts?.discover },
    { key: "favorites", label: labels.favorites, count: counts?.favorites },
    { key: "my-meal-preps", label: labels.myMealPreps, count: counts?.myMealPreps },
  ];
